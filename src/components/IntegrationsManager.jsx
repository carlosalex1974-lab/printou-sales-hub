...
export default function IntegrationsManager({ products, channels, integrationLogs = [], credentials, onWebhookTriggered, onClearLogs }) {
    ...
    const [ml2ClientId, setMl2ClientId] = useState(credentials?.mercadolivre2?.clientId || '');
    ...
    const handleAuthML = (accountKey) => {
        const clientId = accountKey === 'mercadolivre2' ? ml2ClientId : mlClientId;
        if (!clientId) {
            alert('Por favor, insira e salve o Client ID antes de tentar conectar a conta.');
            return;
        }
        const state = accountKey === 'mercadolivre2' ? 'ml2' : 'ml1';
        const redirectUri = encodeURIComponent(`${window.location.origin}/api/auth/mercadolivre/callback`);
                    ...
                    <div className="space-y-3 text-xs">
                        <div className="space-y-1">
                            ...
                            <code className="bg-[#121215] text-brand-orange p-2 rounded-lg block overflow-x-auto text-[10px]">
                                {window.location.origin}/api/webhooks/mercadolivre
                            </code>
                        </div>

                        <div className="border-t border-white/5 pt-3 space-y-2">
                            <span className="text-[10px] text-brand-yellow font-extrabold uppercase tracking-widest block">Conta 1: printoustudio3d (Clássico)</span>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-brand-darkBg border border-brand-borderBg text-white rounded-lg p-2 focus:outline-none focus:border-brand-orange text-xs"
                                    />
                                </div>
                                <div className="space-y-1">
                                    <span className="text-gray-400 font-bold uppercase text-[9px] block">Client Secret</span>
                                    <input
                            ...
                            <div className="flex items-center justify-between pt-1">
                                <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded ${
                                    credentials?.mercadolivre?.status === 'Autorizado' 
                                        ? 'bg-emerald-500/10 text-emerald-400' 
                                        : 'bg-amber-500/10 text-amber-400'
                                }`}>
                                    Status: {credentials?.mercadolivre?.status || 'Pendente'}
                                </span>
                                <button
                                    type="button"
                                    onClick={() => handleAuthML('mercadolivre')}
                                    className="py-1 px-3 bg-brand-orange/10 hover:bg-brand-orange/20 text-brand-orange border border-brand-orange/30 font-bold text-[10px] uppercase tracking-wider rounded-lg transition-all cursor-pointer"
                                >
                                    {credentials?.mercadolivre?.status === 'Autorizado' ? 'Reconectar' : 'Conectar Conta'}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="border-t border-white/5 pt-3 space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1">
                                    <span className="text-gray-400 font-bold uppercase text-[9px] block">Client ID</span>
                                    <input
                                        type="text" value={ml2ClientId} onChange={e => setMl2ClientId(e.target.value)}
                                        placeholder="Ex: 88775544..."
                                        className="w-full bg-brand-darkBg border border-brand-borderBg text-white rounded-lg p-2 focus:outline-none focus:border-brand-orange text-xs"
                                    />
                                </div>
                                <div className="space-y-1">
                                    <span className="text-gray-400 font-bold uppercase text-[9px] block">Client Secret</span>
                                    <input
                                        type="password" value={ml2ClientSecret} onChange={e => setMl2ClientSecret(e.target.value)}
                                        placeholder="••••••••••••"
                                        className="w-full bg-brand-darkBg border border-brand-borderBg text-white rounded-lg p-2 focus:outline-none focus:border-brand-orange text-xs"
                                    />
                                </div>
                            ...
                            <div className="flex items-center justify-between pt-1">
                                <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded ${
                                    credentials?.mercadolivre2?.status === 'Autorizado' 
                                        ? 'bg-emerald-500/10 text-emerald-400' 
                                        : 'bg-amber-500/10 text-amber-400'
                                }`}>
                                    Status: {credentials?.mercadolivre2?.status || 'Pendente'}
                                </span>
                                <button
                                    type="button"
                                    onClick={() => handleAuthML('mercadolivre2')}
                                    className="py-1 px-3 bg-brand-orange/10 hover:bg-brand-orange/20 text-brand-orange border border-brand-orange/30 font-bold text-[10px] uppercase tracking-wider rounded-lg transition-all cursor-pointer"
                                >
                                    {credentials?.mercadolivre2?.status === 'Autorizado' ? 'Reconectar' : 'Conectar Conta'}
                                </button>
                            </div>
                        </div>
                    </div>
