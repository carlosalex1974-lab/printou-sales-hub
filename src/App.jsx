...
const INITIAL_CHANNELS = [
    ...
    { id: 'ml2', name: 'CPS Solu��es', commission: 19.5, fixedFee: 6.0, color: '#FFB900', hasFreeShippingThreshold: true, freeShippingThreshold: 79.0, defaultSellerShippingCost: 19.90, defaultBelowThresholdShippingCost: 0.0 },
    ...
    const calculateProductCost = (prod) => {
        ...
        return parseFloat((rawCost + prod.finishingCost + prod.packagingCost).toFixed(2));
            ...
            const computeSaleFinancials = (sale) => {
                ...
                const productCostTotal = prod ? (typeof calculateProductCost === 'function' ? calculateProductCost(prod) : 0) * qty : 0;
                ...
                if (chan) {
                    ...
                    fee = (sale.grossValue * ((chan.commission || 0) / 100)) + (fixedFee * qty);
                }
                
                let shipping = 0;
                if (sale.channelId === 'direta') {
                    // Venda Direta / Pix: Usa estritamente o valor informado na venda (padrão R$ 0,00)
                    shipping = (sale.shipping !== undefined && sale.shipping !== null && sale.shipping !== '' && !isNaN(parseFloat(sale.shipping)))
                        ? parseFloat(sale.shipping)
                        : 0;
                } else {
                    // Canais Online (Mercado Livre, Shopee, Site):
                    // Determina o custo de frete do vendedor (tabela subsidiada do produto ou do canal)
                    let sellerShippingUnit = (prod && prod.customShippingCost !== undefined && prod.customShippingCost !== null && parseFloat(prod.customShippingCost) > 0)
                        ? parseFloat(prod.customShippingCost)
                        : (chan ? (chan.defaultSellerShippingCost || 20.75) : 20.75);

                    let expectedSellerShipping = sellerShippingUnit * qty;

                    const rawShip = (sale.shipping !== undefined && sale.shipping !== null && sale.shipping !== '' && !isNaN(parseFloat(sale.shipping)))
                        ? parseFloat(sale.shipping)
                        : 0;

                    // Se o valor gravado na venda for desproporcionalmente alto (frete bruto de transportadora R$ 43,50 / R$ 87,00) ou 0, usa o frete real do vendedor
                    if (rawShip > expectedSellerShipping + 0.05 || rawShip === 0) {
                        shipping = expectedSellerShipping;
                    } else {
                        shipping = rawShip;
                    }
                }
